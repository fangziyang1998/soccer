...
def read_match_data(DATADIR):
    '''
    read_match_data(DATADIR,gameid):
    read all Metrica match data (tracking data for home & away teams, and ecvent data)
    '''
    tracking_home = tracking_data(DATADIR,'Home')
    tracking_away = tracking_data(DATADIR,'Away')
    events = read_event_data(DATADIR)
    return tracking_home,tracking_away,events

def read_event_data(DATADIR):
    '''
    read_event_data(DATADIR):
    read Metrica event data  for game_id and return as a DataFrame
    '''
    eventfile = '/Sample_Game_1_RawEventsData.csv' # filename
    events = pd.read_csv('{}/{}'.format(DATADIR, eventfile)) # read data
    return events

def tracking_data(DATADIR,teamname):
    '''
    ...
    '''
    teamfile = '/Sample_Game_1_RawTrackingData_%s_Team.csv' % (teamname)
...
def to_metric_coordinates(data,field_dimen=(100.,60.) ):
    '''
...
def to_single_playing_direction(home,away,events):
    ...
    for team in [home,away,events]:
        second_half_idx = home.Period.eq(2).idxmax()
